let videoDuration = 0;
let audioDuration = 0;
let audioTimelineInfo = {}; // 오디오 타임라인 정보를 저장할 객체 선언

let audioPlayers = {};

// 모든 메뉴 항목과 컨텐츠를 가져옵니다.
const menuItems = document.querySelectorAll('.menu');
const contents = document.querySelectorAll('.content');

// 선택된 메뉴 항목을 강조하기 위한 함수
...
function displayResults(data) {
    ...
    resultsElement.innerHTML = ''; // 이전 결과를 비웁니다.
...
function extractSubstring(str) {
    ...
    if (firstBackslashIndex === -1) {
        return "";  // 백슬래시가 없으면 빈 문자열 반환
    }

    // 두 번째 백슬래시의 위치를 찾습니다.
    let secondBackslashIndex = reversedStr.indexOf('\\', firstBackslashIndex + 1);
    if (secondBackslashIndex === -1) {
        secondBackslashIndex = reversedStr.length;  // 두 번째 백슬래시가 없으면 문자열 끝까지 사용
    }

    // 첫 번째 백슬래시부터 두 번째 백슬래시 사이의 문자열을 추출하고 뒤집습니다.
...
function displayAudioList(data) {
    ...
    listElement.innerHTML = ''; // 기존 리스트 아이템들을 초기화

    data.forEach(function(item) {
        // 각 항목에 대한 HTML 문자열을 생성
        var listItemHTML = `
            <li>
                ${item.id}
                <audio controls>
...
function updateAudioClipPath(fileName) {
    ...
    // response.audio_clips_info 배열에서 일치하는 class 이름 찾기
    response.audio_clips_info.forEach(clip => {
...
function addAudioTimeline(duration, fName, startTime, index) {
    ...
    if (fileName.includes('train')) {
        audioTimeline.style.backgroundColor = '#007bff'; // 예시 색상
    } else if (fileName.includes('wheel')){
        audioTimeline.style.backgroundColor = '#d5ca01'; // 다른 색상
...
document.getElementById('merge').addEventListener('click', function () {
    // 수정된 audio_clips_info 배열을 사용하여 요청 본문 구성
...
window.onload = function() {
    ...
    new Chart(document.getElementById('chart-subscription'), {
        ...
        options: {
            aspectRatio: 1.2 // 이 값을 변경하여 다른 비율로 조정할 수 있음
        }
    });
    ...
    // 리스트 추가
    var list = document.getElementById('list-mostused'); // 이 ID가 ol 태그의 ID와 일치해야 합니다.
    mostUsedSounds.forEach(function(sound, index) {
        var listItem = document.createElement('li');
        listItem.innerText = sound; // 숫자는 ol 태그에 의해 자동으로 추가됩니다.
        list.appendChild(listItem);
    });

    // 리스트 추가
    var list = document.getElementById('list-review'); // 이 ID가 ol 태그의 ID와 일치해야 합니다.
